...
function WritableStream(connection, id) {
  ...
  this._heartbeat.on("beat", this._connection.emit.bind(this._connection, CHANNEL_TYPE, id, "__writebeat", null));
  this._heartbeat.on("timeout", this._handleTimeout.bind(this));
  this._connection.on(CHANNEL_TYPE, id, "__readbeat", this._heartbeat.received.bind(this._heartbeat));
...
WritableStream.prototype._handleTimeout = function () {
  this.emit("timeout");
  this.emit("error", new TimeoutError("Lost connection to downstream"));
};
